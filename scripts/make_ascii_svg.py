...
CONTRAST = 1.05
BRIGHTNESS = 1.0
GAMMA = 1.22          # brighten mids so features separate instead of reading uniformly dense
SHARPEN = False        # sharpening was adding noise, making the grid read muddy
WHITE_FLOOR = 0.84    # clean background back to blank, keep some floor for skin
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">aqib@github: ~$ ./portrait.sh</text>')
...
status_prefix = "aqib@github:~$ whoami "
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{status_prefix}<tspan fill="{INK}">Aqib Amin</tspan></text>')
cursor_x = PAD + len(status_prefix) * 8.9
